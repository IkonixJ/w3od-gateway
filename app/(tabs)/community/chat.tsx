...
export default function ChatScreen() {
  ...
  const [otherUser, setOtherUser] = useState<{ username: string | null; display_name: string | null; avatar_url: string | null; email_verified: boolean } | null>(null);
  const scrollRef = useRef<ScrollView>(null);
  const typingTimer = useRef<ReturnType<typeof setTimeout> | null>(null);

  const loadMessages = useCallback(async () => {
    if (!id) return;
    const data = await getConversationMessages(id, 100, 0);
    setMessages(data);
    setLoading(false);
    ...
    if (data.length > 0 && profile?.id) {
      ...
      if (otherMsg) {
        setOtherUser({
          ...
          display_name: otherMsg.sender_display_name,
          avatar_url: otherMsg.sender_avatar_url,
  ...
  useEffect(() => {
    loadMessages();
    if (!id) return;
    const unsub = subscribeToConversationMessages(id, () => {
      loadMessages();
    });
    return unsub;
  }, [id, loadMessages]);
  ...
  useEffect(() => {
    if (!id) return;
    const unsub = subscribeToTyping('dm', id, (userId) => {
  ...
  const handleSend = useCallback(async () => {
    ...
    setInput('');
    await clearTyping('dm', id);
    const result = await sendMessage(id, body, 'text', null, replyTo?.id ?? null);
    setSending(false);
    if (result.success) {
      setReplyTo(null);
      loadMessages();
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <View style={styles.headerMeta}>
            <Avatar uri={otherUser?.avatar_url} displayName={otherUser?.display_name ?? otherUser?.username} size="sm" />
            <View style={styles.headerNameWrap}>
              <View style={styles.headerNameRow}>
                <NeonText variant="body" weight="semiBold" tone="cyan" style={styles.headerName} numberOfLines={1}>
                  {otherUser?.display_name ?? otherUser?.username ?? 'Member'}
                </NeonText>
                {otherUser?.email_verified && <ShieldCheck color={Palette.neonLime} size={12} />}
              </View>
              {typingUser ? (
                ...
                <NeonText variant="body" tone="muted" style={styles.headerSub}>
                  {otherUser?.username ? `@${otherUser.username}` : 'Direct message'}
                </NeonText>
              )}
            </View>
          </View>
        ...
        {replyTo && (
          <View style={styles.replyPreview}>
            <View style={styles.replyLeft}>
              <Reply color={Palette.neonCyan} size={14} />
              <View style={styles.replyMeta}>
                <NeonText variant="body" tone="muted" style={styles.replyLabel}>
                  Replying to
                </NeonText>
                <NeonText variant="body" weight="semiBold" tone="cyan" style={styles.replyText} numberOfLines={1}>
        ...
        <View style={styles.inputBar}>
          <Pressable onPress={() => setShowEmojis((s) => !s)} hitSlop={10} style={styles.inputIconBtn}>
            <Smile color={Palette.neonCyan} size={22} />
          </Pressable>
          <Pressable onPress={handleFileUpload} hitSlop={10} style={styles.inputIconBtn}>
            <Paperclip color={Palette.neonCyan} size={22} />
          </Pressable>
      ...
      <View style={[styles.bubble, isMe ? styles.bubbleRight : styles.bubbleLeft]}>
        ...
        {message.reply_to && (
          <View style={styles.replyContext}>
            ...
            <NeonText variant="body" tone="muted" style={styles.replyContextText} numberOfLines={1}>
              Reply
            </NeonText>
          </View>
        )}
        ...
        {message.message_type !== 'text' && message.media_url && (
          <View style={styles.mediaBox}>
            {message.message_type === 'image' ? (
              <ImageIcon color={Palette.neonCyan} size={24} />
            ) : (
              <Paperclip color={Palette.neonCyan} size={24} />
            )}
            <NeonText variant="body" tone="cyan" style={styles.mediaLabel}>
        ...
        <View style={styles.bubbleFooter}>
          ...
          {isMe && message.read_at && <CheckCheck color={Palette.neonCyan} size={12} />}
          {isMe && !message.read_at && <Check color={Palette.textTertiary} size={12} />}
        </View>
